...
export async function callLLM(event, file, overrides = {}) {
    const {runFetch = fetch} = overrides;
    const res = await runFetch(GROQ_URL, {
        method: 'POST',
        
        headers: {
            'Authorization': `Bearer ${process.env.TYPOS_AI_KEY}`,
